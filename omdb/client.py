...
class OmdbMovie:
    """ A simple class to represent movie data coming back from OMDb and 
    transform to Python types."""
    ...
    @property
    def imdb_id(self): return self.data['imdbID']

    @property
    def title(self): return self.data['Title']

    @property
    def year(self): return int(self.data['Year'])
...
class OmdbClient:
    ...
    def make_request(self,params):
        # Make a get request to the API adding the APIKEY to the params
    ...
    def search(self,search):
        """Search for movies by title. This is a generator so all
        results from all pages will be iterated across"""
        page,seen_results,total_results=1,0,None
